                  ...
                  )
{
    ...
    // 跟踪调试的接收信息处理：回应信息记录。
	int n=0;
		printf("ReceiveString采集到的数据:\n");
	for(n=0;n<lRead;n++)
	{
			printf("%02X ", recvChar[n]);
	}
	printf("\n");
#ifndef _TSR_
	if( bTestFlag  )
...
{    
	...
	unsigned char nSendBuf[][20] =
    {
		...
		{ 0x00,    0x03,    0x00, 0x00,   0x00,  20,   0x00, 0x00 },   //[5]表示读取寄存器个数(一般一个寄存器2byte) 2字节的数据通道（通道数为该【5】值）
	...
	for(i=0;i<10;i++)
		{
			printf("打印采集sampler的数据buf[%d]=%d   %f\n", i, pData[i], pData[i]);
		}
...
bool Report( HANDLE hComm,  float *fdata,  char* sRecStr) //控制命令#开头
{
	...
	if( (sRecStr!=NULL)&&(SstrBuf[0] != '$'))
	{
		...
		printf("libUpdateEquipt.22222>>Report>>> SstrBuf=%s\n", SstrBuf);
	...
	int iAddr = fdata[0];
//	printf("libUpdateEquipt.so>>Report>>> RdataBuf[%d]))3)=%f  %f  %f  %f\n", iAddr, 
//										RdataBuf[iAddr][0],RdataBuf[iAddr][1],RdataBuf[iAddr][2],RdataBuf[iAddr][3]);
/*    printf("libUpdateEquipt2222.so>>Report>>>获取到的数据：\n");
	for(i=0; i<50; i++)
	{
		printf("%f ", RdataBuf[iAddr][i]);
	}
	printf("\n\n");
*/
	return TRUE;
				...
				if(SAddr>-1 && ByteLenth>-1 && SAddr+ByteLenth/2<ReportMaxSignalNum) //4.验证信号地址 //5.验证信号长度
					{
						...
						for(n=0;n<len;n++)
						{
									float f_data = RdataBuf[EAddr-1][SAddr+n];
