...
key_t SHMM_Open(int bCreate)
{
	...
	{// 如果是创建共享内存，则初始化共享内存区
		...
		// Attach the segment 
		shmbuf = (char *)shmat(g_iShmmId, NULL, 0);



		
		if(shmbuf == (char *)-1)
		//if((shmbuf = shmat(g_iShmmId, NULL, 0)) ==  -1)
		{
			// 信号灯解锁
...
int SHMM_SetReadedStatus()
{
	...
	/* 
	 * add other 8 channel DI
	 * reason:
	 *		estone have 9 DI 
	 *
	 *		modifiy by Aaron.Xu 2009-03-10
	 */
	memcpy(shmbuf + 3*sizeof(char), &cDiWarnStatus, sizeof(cDiWarnStatus));
	// Detach the segment 
    if (shmdt(shmbuf) < 0)
	{
		// 信号灯解锁
