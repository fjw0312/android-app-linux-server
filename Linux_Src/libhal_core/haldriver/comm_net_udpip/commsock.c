 ...
 * RETURN   : static int : 
 * COMMENTS : 
 * CREATOR  : Frank Mao                DATE: 2004-09-13 20:03
 *==========================================================================*/
								  ...
								  IN DWORD dwServerAddr, IN short nServerPort )
{
	...
	sAddr.sin_addr.s_addr = inet_addr("142.100.6.24");
					  ...
					  IN int nTimeout )
{
    ...
    char cMsg[256];
    sprintf( cMsg, "dwServerAddr=%d  nServerPort=%d\n", (int)dwServerAddr, (int)nServerPort);
    puts( cMsg );
    sprintf( cMsg, "SOCK_STREAM=%d  SOCK_DGRAM=%d\n", (int)SOCK_STREAM, (int)SOCK_DGRAM);
    puts( cMsg );
    ...
    sAddr.sin_family      = AF_INET ;
	sAddr.sin_port        = htons((u_short)nServerPort);
	//sAddr.sin_port        = htons((u_short)6789);
	//sAddr.sin_addr.s_addr = htonl(dwServerAddr);
	//sAddr.sin_addr.s_addr = inet_addr("142.100.6.23");
	sAddr.sin_addr.s_addr = htonl(INADDR_ANY);

    rc = bind ( s, (struct SC_SOCKADDR *)&sAddr, sizeof( sAddr ) );
    if( rc == 0 )
    {
        puts("+++++++++++++++++++成功！*******************************");
    }
...
//#define COMM_CLIENT_MODE	0x01
    ...
    char cMsg[256];
    sprintf( cMsg, "pPortDescriptor=%s\n", pPortDescriptor);
    puts( cMsg );
    sprintf( cMsg, "pOpenParams=%s\n", pOpenParams);
    puts( cMsg );
    sprintf( cMsg, "dwPortAttr=%d  nTimeout=%d\n", (int)dwPortAttr, nTimeout);
    puts( cMsg );
    sprintf( cMsg, "COMM_CLIENT_MODE=%d  SOCK_DGRAM=%d\n", (int)COMM_CLIENT_MODE, (int)SOCK_DGRAM);
    puts( cMsg );
...
//			SOCK_STREAM,
        ...
        //sendto(pPort->nSocket, "12331ABCDEFGHIJKLMNOPQRSTUVWXYZ", 26, 0, 
        //(struct sockaddr*)&pPort->inPeerAddr, sizeof(pPort->inPeerAddr));  
        
		//rc = sendto(fd, pBuffer, nBytesToWrite, 0, 
		//	(struct sockaddr *)to, &nToLen);

        //sprintf(cMsg, "CommOpen:pPort=%d  sin_port=%d sin_port=%X s_add=%08X\n", 
        //    pPort->nSockPort, pPort->inPeerAddr.sin_port, pPort->inPeerAddr.sin_port, pPort->inPeerAddr.sin_addr.s_addr);
        //puts(cMsg);
	}
...
int TCPIP_CommWrite(IN HANDLE hPort, IN char *pBuffer,	IN int nBytesToWrite)
{
	...
	int				nToLen = sizeof(pPort->inPeerAddr);
#endif
    ...
    while (nBytesToWrite > 0)
    {
        ...
        //sprintf(cMsg, "%s:s_addr=%08X port=%d\n", __FUNCTION__, to->sin_addr.s_addr, to->sin_port);
        //puts(cMsg);
        
        //AppLogOut("%s:s_addr=%08X\n", __FUNCTION__, m_address.sin_addr.s_addr);

        //rc = sendto(pPort->nSocket, pBuffer, nBytesToWrite, 0, 
        //(struct sockaddr*)&pPort->inPeerAddr, sizeof(pPort->inPeerAddr));  
 ...
 *
 * Create       : Mao Fuhua    2001-3-1 8:17:14
 * Comment(s)   : 
 *--------------------------------------------------------------------*/
 ...
 * Return type  : int , 0: time out, -1: error, > 0: the value of nLen
 *
 * Create       : Mao Fuhua    2001-3-1 8:17:18
 * Comment(s)   : 
 *--------------------------------------------------------------------*/
