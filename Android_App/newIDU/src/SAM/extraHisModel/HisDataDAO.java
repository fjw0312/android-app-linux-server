...
public class HisDataDAO {
	...
	public static void saveHisEquip(){
		...
		//遍历链表
			for(int i=0; i<hisEquipId_lst.size(); i++){ 
				...
				Hashtable<Integer, Signal> signal_ht = NetDataModel.getHtSignal(id);
				if(signal_ht==null) continue;
				String buf[] = new String[signal_ht.size()];
				String date = "";
				int j = 0;
				Iterator<Integer> sigId_lst = signal_ht.keySet().iterator();
				while(sigId_lst.hasNext()){
					int sigId = sigId_lst.next();
					Signal sig = signal_ht.get(sigId);
					HisSignal hisSignal = new HisSignal();
					hisSignal.equipid = hisEquipId_lst.get(i);
					int EquiptId = Integer.parseInt(hisSignal.equipid);
					hisSignal.equip_name = NetDataModel.getEquipmentCfg(id).EquipTemplateName; //*
					hisSignal.sigid = String.valueOf(sig.sigId);
					int SignalId = Integer.parseInt(hisSignal.sigid);
					hisSignal.name = sig.name; //*
					hisSignal.value = sig.value;
					hisSignal.mean = sig.meaning;
					hisSignal.value_type = String.valueOf(sig.type);
					hisSignal.is_invalid = String.valueOf(sig.invalid);
					hisSignal.severity = String.valueOf(sig.severity);
					hisSignal.freshtime = String.valueOf(sig.readtime);
	...
	public static void saveHiSignal(){
		...
		for(int i=0; i<hisSignalId_lst.size(); i++){
			...
			Signal sig = NetDataModel.getSignal(equipId, signalId);
			HisSignal hisSignal = new HisSignal();
			hisSignal.equipid = ids[0];
			int EquiptId = Integer.parseInt(hisSignal.equipid);
			hisSignal.equip_name = NetDataModel.getEquipmentCfg(equipId).EquipTemplateName; //*
			hisSignal.sigid = ids[1];
			int SignalId = sig.sigId;
			hisSignal.name = sig.name; //*
			hisSignal.value = sig.value;
			hisSignal.mean = sig.meaning;
			hisSignal.value_type = String.valueOf(sig.type);
	...
	public static void SaveHisFormula(int PathMode){
		...
		for(int i=0; i<hisFormulaId_lst.size(); i++){
			...
			for(int j=0; j<str.length; j++){
				...
				hisFormula.add_string( sig.value );
				hisFormula.getTime = String.valueOf(sig.readtime);
