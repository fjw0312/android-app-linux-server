...
//本包的逻辑parseThread-》parseEquuipt-》xmlCfg-》本地xml文件  ==》xmlDataModel数据存放被访问类
public class parseThread extends Thread{
	...
	public void run() {
		...
		for(int i=0;i<filesNum;i++){
			parseFileThread thread = new parseFileThread(XmlFile+"/"+strfiles[i]);
			thread.start();
		//	Log.e("parseThread>>文件：",strfiles[i]);
		}			
	}

	public class parseFileThread extends Thread{
		public parseFileThread(String fileName){
			filename = fileName;
		}
		//Fields
		String filename = "";
