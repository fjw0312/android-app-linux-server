...
//自定义控件  年用电量     柱状图
public class Ks_zHisBarChart_RcYear extends ViewGroup implements VObject{

	public Ks_zHisBarChart_RcYear(Context context) {
		super(context); 
...
//						Log.e("Ks_zHisBarChart_RcYear>>onDraw>>str", point_lst.get(i));
						if(i > numDay-1) break;
						int ii = i;
			...
			Log.e("Ks_zHisBarChart_RcYear>>onDraw>>","绘制曲线 异常抛出！");
		}	
...
//		Log.e("Ks_zHisBarChart_RcYear-doLayout","into");
	...
	public boolean  updataValue(String strValue) {
		...
		if(HisDataDAO.getPueLine_HisFormulaList(nowfileName,13, 1)){
			 List<HisFormula> hisFormula_lst = HisDataDAO.year_RCBar_hisFormula_lst;			
			...
			for(int i=0; i<hisFormula_lst.size();i++){
					...
					if(value_l_time > prev_num_Mon){	//最近num个月
						...
						String readTime_day = hisFormula.strTime.substring(0,4); //获取 年月 
...
//						Log.e("Ks_zHisBarChart_RcYear>>updateValue>>strPoint：", hisFormula_lst.get(i).strContent+"---"+hisFormula_lst.get(i).strEndContent+">>>"+strPoint);
					}
			}//end for
		}//end if
