...
//自定义控件Oval-Ellipse  使用new TextView方式  目前 未处理渐变色  数值绑定 颜色改变功能
public class Ks_Oval extends ViewGroup implements VObject{

	public Ks_Oval(Context context) {
		super(context);
		//实例化该控件的组合元素控件
	...
	//还支持绑定信号告警等级"Binding{[Event[Equip:6-Temp:178-Event:5]]}"
...
//		canvas.drawColor(Color.GREEN);   //设置viewgroup的底板颜色 
		...
		rectf.left = 0+v_iBorderWidth/2+1;
		rectf.top = 0+v_iBorderWidth/2+1;
		rectf.right = v_iWidth-v_iBorderWidth/2-1;
		rectf.bottom = v_iHeight-v_iBorderWidth/2-1;   
		//绘制矩形填充颜色
