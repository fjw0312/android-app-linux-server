...
//自定义控件  日用电量pue     柱状图
public class Ks_zHisBarChart_PueDay extends ViewGroup implements VObject{

	public Ks_zHisBarChart_PueDay(Context context) {
		super(context); 
	...
	{		
		...
		try{
			...
			//			Log.e("Ks_BarChart>>onDraw>>str", point_lst_mon.get(i));
						//提取 采集时间 和 数值
						if(i > numDay-1) break;						
						int ii = i;
			...
			Log.e("Ks_HisBarChart_PueChart>>onDraw>>","绘制曲线 异常抛出！");
		}
...
//		Log.e("Ks_zHisBarChart_RcYear-doLayout","into");
	...
	public boolean  updataValue(String strValue) {
		...
		if(HisDataDAO.getPueLine_HisFormulaList(nowfileName, 11, 2)){
			List<HisFormula> hisFormula_lst = HisDataDAO.day_pueBar_hisFormula_lst;
			if(hisFormula_lst==null) return false; 			
				...
				for(int i=0; i<hisFormula_lst.size();i++){
					...
					if(value_l_time > prev_num_Day){	//最近4 天
						...
						float  f_value = f_start_value_T/(f_start_value_T-f_start_value_H);
			...
			//			Log.e("Ks_HisBarChart_PueChart>>updateValue>>strPoint：", hisFormula_lst.get(i).strContent+"---"+hisFormula_lst.get(i).strEndContent+">>>"+strPoint);
					}
				}//end for
		}//end if
