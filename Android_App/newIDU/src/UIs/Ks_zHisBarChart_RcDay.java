...
//自定义控件  日用电量     柱状图
public class Ks_zHisBarChart_RcDay extends ViewGroup implements VObject{

	public Ks_zHisBarChart_RcDay(Context context) {
		super(context); 
	...
	{		
		...
		try{
			...
			Log.e("Ks_HisBarChart_Day>>onDraw>>","绘制曲线 异常抛出！");
		}
...
//		Log.e("Ks_zHisBarChart_RcYear-doLayout","into");
	...
	public boolean  updataValue(String strValue) {
		...
		prev_num_Day = nowTime - (long)numDay*24*3600*1000; //获取numDay周天前的 时间		
		...
		if(HisDataDAO.getPueLine_HisFormulaList(nowfileName,11, 1)){
			List<HisFormula> hisFormula_lst = HisDataDAO.day_RCBar_hisFormula_lst;			
			...
			for(int i=0; i<hisFormula_lst.size();i++){
					...
					if(value_l_time > prev_num_Day){	//最近4 天
						...
						int TH_MON = 1; 
						if(TH_MON==1){       //总用电量
							f_value = f_end_value_T - f_start_value_T;
						}else if(TH_MON==2){ //空调/其他 用电量
							f_value = f_end_value_H - f_start_value_H;
						}else if(TH_MON==3){ //IT 用电量
							f_value = (f_end_value_T - f_end_value_H) - (f_start_value_T - f_start_value_H);
						}
						

						String readTime_day = hisFormula.strTime.substring(0,10); //获取 天 
			...
			//			Log.e("Ks_HisBarChart_Day>>updateValue>>strPoint：", hisFormula_lst.get(i).strContent+"---"+hisFormula_lst.get(i).strEndContent+">>>"+strPoint);
					}
			}//end for
		}//end if
