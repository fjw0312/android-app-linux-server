...
//自定义控件  月用电量pue   柱状图
public class Ks_zHisBarChart_PueMon extends ViewGroup implements VObject{

	public Ks_zHisBarChart_PueMon(Context context) {
		super(context); 
	...
	{		
		...
		try{
			...
			Log.e("Ks_HisBarChart_PueMon>>onDraw>>","绘制曲线 异常抛出！");
		}
...
//		Log.e("Ks_zHisBarChart_RcYear-doLayout","into");
	...
	public boolean  updataValue(String strValue) {
		...
		//获取信号 当月 历史式子数据
		nowTime = java.lang.System.currentTimeMillis(); //每个月按30天计算
		...
		if(HisDataDAO.getPueLine_HisFormulaList(nowfileName, 12, 2)){
			List<HisFormula> hisFormula_lst = HisDataDAO.mon_pueBar_hisFormula_lst;
			if(hisFormula_lst==null) return false; 
				...
				for(int i=0; i<hisFormula_lst.size();i++){
					...
					if(value_l_time > prev_num_Mon){	//最近4 月
						...
						String readTime_day = hisFormula.strTime.substring(0,7); //获取 月 						
		...
		//				Log.e("Ks_HisBarChart_PueMon>>updateValue>>strPoint：", hisFormula_lst.get(i).strContent+"---"+hisFormula_lst.get(i).strEndContent+">>>"+strPoint);
					}
				}//end for
		}//end if
