...
//		public static int r_line_num = 0;  //文件已经读取的信号行数
	...
	//判断文件是否为空等
	public boolean has_file(String filename ){
...
*/		
	...
	//修改含有某字符串 的整行字符
	public boolean exchange_str_line(String pathName,String str, String newline){

		String buff = "";
		try{
			 File file1 = new File(pathName);
			 File file2 = new File(pathName+"txt");
			 File file3 = new File(pathName);
			 file2.createNewFile(); //新建目录/文件
			 
			 priwrite = new PrintWriter(new FileWriter(file2,true),true); //true尾部写入行字符串 
			 bufread = new BufferedReader( new FileReader(file1));
...
//			buflist1 = new ArrayList<String>();
	...
	//读取含有某些字符的  所有行字符 如果含有某字符串-整个字符串添加进链表输出，否则输出空链表
		...
		//读取某文件 的某行字符
		public String read_File_str_line(String fileName,String buf){
			buf_str_line = "";
			String buff = "";
			try{
